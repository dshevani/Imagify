...
SENNA_Tokens* SENNA_Tokenizer_tokenize(SENNA_Tokenizer *tokenizer, const char *sentence)
{
  ...
  /* first replace all numbers by '0' and keep them for tokens */
  sentence_size = strlen(sentence)+1;
  if(sentence_size > tokenizer->max_sentence_size)
  {
    tokenizer->max_sentence_size = sentence_size;
    tokenizer->sentence = SENNA_realloc(tokenizer->sentence, sizeof(char), sentence_size);
  }
  
  reader = sentence;
  writer = tokenizer->sentence;
  while(1)
  {
    char c = *reader;
    int number_size = 0;
    ...
    if(isdigit(c) || c == '+' || c == '-' || c == '.' || c == ',')
      SENNA_tokenize_number(&number_size, reader);
  ...
  while(1)
  {
    ...
    {
      SENNA_tokenize_untilspace(&sizetoken, sentence);
      SENNA_tokenize_dictionarymatch(&sizedictionary, &idxhash, tokenizer->word_hash, sentence);
    ...
    {
      SENNA_tokenize_alphanumeric(&sizealphanumeric, sentence);
      SENNA_tokenize_dictionarymatch(&sizedictionary, &idxhash, tokenizer->word_hash, sentence);
    ...
    /* word strings */
    {
      ...
      if(sizetoken >= tokenizer->words_sizes[tokenizer->tokens->n])
      {
        tokenizer->words_sizes[tokenizer->tokens->n] = sizetoken+1;
      ...
      memcpy(tokenizer->tokens->words[tokenizer->tokens->n], sentence, sizetoken);
      tokenizer->tokens->words[tokenizer->tokens->n][sizetoken] = '\0';
    }
    ...
    if(tokenizer->caps_hash)
    {
      ...
      allcaps = !islower(sentence[0]);
      initcap = isupper(sentence[0]);
      hascap = initcap;
    ...
    if(tokenizer->suff_hash)
    {
      ...
      suffix[0] = (char)(sizetoken >= 2 ? tolower(sentence[sizetoken-2]) : (sizetoken >= 1 ? tolower(sentence[sizetoken-1]) : '\0'));
      suffix[1] = (char)(sizetoken >= 2 ? tolower(sentence[sizetoken-1]) : '\0');
